...
class IndexedColorFilter(initialImage: java.util.List[Coordinate]) extends KMeans:
  ...
  private val newMeans = kMeans(points, means, 0.1)
  println("gets here")
...
//  }
  ...
  private def indexedImage(img: java.util.List[Coordinate], means: Seq[Point]) =
//    val dst = Img(img.width, img.height)
    val pts = collection.mutable.Set[Point]()

    img.forEach(coordinate => {
